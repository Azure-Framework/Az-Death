...
-- Eligibility (so you can call when injured)
Config.AIEMS.Eligibility = Config.AIEMS.Eligibility or {
  HealthThreshold = 175,     -- if hp <= this, allow
  RecentDamageMs  = 45000,   -- if damaged in last 45s, allow
  AllowWhenDowned = true,    -- allow when dead/dying
  AlwaysAllowIfUiShowsInjury = true,
}

-- Chance of death (used after AI treatment)
...
-- Injury UI export reader (optional)

local function getInjuryUi()
  -- assumes your injury system export is in the SAME resource
...
local function parseUiInjurySignals()
  -- returns: hasUiInjury, bleedLevel, woundCount, gswTorso, gswHead
  ...
  while true do
    ...
    if ped ~= 0 and DoesEntityExist(ped) then
      if HasEntityBeenDamagedByAnyPed(ped) or HasEntityBeenDamagedByAnyVehicle(ped) or HasEntityBeenDamagedByAnyObject(ped) then
        ...
        ClearEntityLastDamageEntity(ped) -- second call helps some cases
      end
    end
  end
end)
...
-- ✅ FAR ROAD SPAWN (VALID ROAD NODE)

local function isWaterAt(pos)
  -- Best-effort water check
  local waterZ = 0.0
...
local function findSpawnFarOnRoad(pCoords)
  ...
  -- Last resort: closest node (may be near)
...
-- Hospital check-in zones

CreateThread(function()
  while true do
    Wait(0)
    local ped = PlayerPedId()
    if ped == 0 or not DoesEntityExist(ped) then Wait(500) goto cont end
    ...
    if not best or bestDist > ((best.radius or 2.2) + 6.0) then
      Wait(350)
      goto cont
    end
