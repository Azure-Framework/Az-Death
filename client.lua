...
Config.ShowAliveInjuries = (Config.ShowAliveInjuries == true) -- default false
Config.ShowDeadInjuries  = (Config.ShowDeadInjuries ~= false) -- default true
...
-- Damage types (your UI just shows details, so we can be rich here)
local DAMAGE_TYPES = {
  GSW       = "GSW",
  BRUISE    = "Bruising",
...
-- Known GTA “vehicle damage weapons”
...
local ASPHYXIA_WEAPON = {
  ...
  [`WEAPON_SMOKEGRENADE`] = false, -- left false intentionally (gas is not always used)
}
...
-- ---------------------------
-- Injury State
-- injuries[region] = { severity=0..100, bleed=0..8, wounds=[...] }
-- ---------------------------
local injuries = {}
local pinned = false
...
-- NOTE: This is the key fix:
-- we pass Config.ShowAliveInjuries instead of always-true
...
-- ---------------------------
-- Primary damage event (great for GSW, melee, fall, vehicle weapon)
-- ---------------------------
...
-- ---------------------------
-- Vehicle/impact fallback
-- Detect health drop + collision/ragdoll and create IMPACT/FRACTURE wounds.
-- ---------------------------
...
-- ---------------------------
-- Fall / hard landing fallback
-- Captures sky-falls even when GTA damage events fail to provide a usable weapon hash.
-- ---------------------------
...
CreateThread(function()
  while true do
    ...
    local ped = PlayerPedId()
    if ped == 0 or not DoesEntityExist(ped) then goto cont end
    if IsEntityDead(ped) or isDowned then goto cont end
...
-- ---------------------------
-- Effects (movement / combat / visuals)
-- ---------------------------
...
CreateThread(function()
  while true do
    ...
    if dead then
      -- still update UI sometimes while downed
      Wait(200)
      sendUiUpdate()
      goto cont
    end
    ...
    -- Stumble chance from overall severity
...
-- ---------------------------
-- Bleeding tick (HP loss + visual blood)
-- ---------------------------
...
CreateThread(function()
  while true do
    ...
    local b = totalBleed()
    if b <= 0.0 then goto cont end

    if (now() - lastBleedTick) >= (Config.BleedTickMs or 1000) then
      lastBleedTick = now()
...
-- ---------------------------
-- Command binding (RECLAIM /injuries)
-- ---------------------------
...
-- ---------------------------
-- Receive state from server (optional sync)
-- ---------------------------
...
CreateThread(function()
  while true do
    ...
    local ped = PlayerPedId()
    if ped == 0 or not DoesEntityExist(ped) then goto cont2 end
    local pcoords = GetEntityCoords(ped)
